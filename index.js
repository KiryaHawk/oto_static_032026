...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        ...
        if (isBlue && !showGibdd) return false;

        const objCategories = extractCategories(obj);

        // Категориальный фильтр:
        // 1) ничего не выбрано -> показываем все
        // 2) выбраны все -> показываем все
        // 3) иначе объект должен содержать ВСЕ выбранные категории
        ...
        // Для синих точек количество не фильтруем
